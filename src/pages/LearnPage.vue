<template>
  <q-page class="space-y-4 py-4 px-8 bg-gray-50">
    <div v-if="!noFlashcards">
      <div
        v-if="flashcards.length > 0"
        class="flex items-center justify-center"
      >
        <flash-card-with-action
          v-model="visibleFlashcard"
          :flashcards="flashcards"
          @remembered="remembered"
          @need-learn="needLearn"
          @ignore="ignore"
        ></flash-card-with-action>
      </div>
      <div v-else class="flex flex-col items-center justify-center h-[60vh]">
        ...
        <button
          @click="refreshPage"
          type="button"
          class="animate-zoom text-white bg-green-700 hover:bg-green-800 focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-1.5 focus:outline-none"
        >
          Continue learning ?
        </button>
      </div>
    ...
    >
      <p class="text-xl text-slate-400">No flashcards to study</p>
    </div>
  </q-page>
</template>
...
import FlashCardWithAction from 'src/components/study_set/FlashCardWithAction.vue';
import StudysetService from 'src/services/studyset';
import { Flashcard } from 'src/types/studyset';

export default defineComponent({
  components: { FlashCardWithAction },
  setup() {
    const flashcards = ref<Flashcard[]>([]);
    const visibleFlashcard = ref(true);
    const studySetID = ref(-1);
    const noFlashcards = ref(false);

    onMounted(async () => {
      const result = await StudysetService.getLearnData();
      studySetID.value = result.studySet;
      flashcards.value = result.flashcards;
      if (result.flashcards.length <= 0) {
        noFlashcards.value = true;
      }
    });

    const remembered = async (flashcard: Flashcard) => {
      await StudysetService.updateLearnData(
        StudysetService.STATUS.REMEMBERED,
        studySetID.value,
        flashcard
      );
    ...
    const needLearn = async (flashcard: Flashcard) => {
      await StudysetService.updateLearnData(
        StudysetService.STATUS.NEED_LEARN,
        studySetID.value,
        flashcard
      );
    ...
    const ignore = async (flashcard: Flashcard) => {
      await StudysetService.updateLearnData(
        StudysetService.STATUS.IGNORE,
        studySetID.value,
        flashcard
      );
